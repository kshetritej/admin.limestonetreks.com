...
function wouldCreateCycle(
  ...
  while (map.has(current)) {
    ...
    if (visited.has(current)) break; // unrelated cycle, stop
    visited.add(current);
      ...
      <div className="w-full  space-y-4">
        ...
        <div className="mb-8">
          ...
          <p className="text-zinc-500 text-sm mt-1">
            Add or update URL redirects. Existing paths will be updated.
          </p>
        </div>
        ...
        <div className="flex gap-3 items-center">
          <Input
            ...
            className="border-zinc-300 text-zinc-900 placeholder:text-zinc-400 focus-visible:ring-zinc-400 h-11 flex-1 text-sm"
            onKeyDown={(e) => e.key === "Enter" && handleSave()}
          />
          <ArrowRight className="text-zinc-400 shrink-0" size={18} />
          <Input
            placeholder="/to-path"
            value={to}
          ...
          <Button
            onClick={handleSave}
            disabled={loading}
            className="bg-zinc-950 text-white hover:bg-zinc-800 h-11 px-5 shrink-0 font-medium rounded-xl"
          >
